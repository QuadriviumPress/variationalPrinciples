...
where $G$, $H$, and $Y$ are functions of the canonical variables plus time. Jacobi’s identity; \ref{15.17} states that the sum of the cyclic permutation of the double Poisson brackets of three functions is zero. Jacobi’s identity plays a useful role in Hamiltonian mechanics as will be shown.
...
The Poisson brackets are invariant under a canonical transformation from one set of canonical variables $(q_k, p_k)$ to a new set of canonical variables $(Q_k, P_k)$ where $Q_k \rightarrow Q_k(\mathbf{q}, \mathbf{p})$ and $P_k \rightarrow P_k(\mathbf{q}, \mathbf{p})$. This is shown by transforming Equation \ref{15.12} to the new variables by the following derivation
...
Let $F = Q_k$ and replace $G$ by $F$, and use the fact that the fundamental Poisson brackets $\{Q_k, Q_j \}_{qp} = 0$ and $\{Q_k, P_j \}_{qp} = \delta_{jk}$, then Equation \ref{15.25} reduces to
...
Substituting equations \ref{15.29} and \ref{15.31} into Equation \ref{15.27} gives
...
Example 15.1: Check that a transformation is canonical
...
These two Poisson Brackets for three functions can be used to derive the Poisson Bracket of four functions, taken in pairs. This can be accomplished two ways using either Equation \ref{15.33} or \ref{15.34}.
...
These two alternate derivations give different relations for the same Poisson Bracket. Equating the alternative equations \ref{15.35} and \ref{15.36} gives that
...
Any observable $G(p, q, t)$ will be a constant of motion if $\frac{dG}{ dt} = 0$, and thus Equation \ref{15.45} gives
...
then Jacobi’s identity, Equation \ref{15.17}, can be used to imply that
...
In Equation \ref{15.45} assume that $G$ is a fundamental coordinate, that is, $G \equiv q_k,$. Since $q_k$ is not explicitly time dependent, then
...
Example 15.5: Two-dimensional, anisotropic, linear oscillator
...
Using the Poisson Bracket expression for the time dependence, Equation \ref{15.45}, and using the fact that the Hamiltonian is not explicitly time dependent, that is, $\frac{\partial H}{ \partial t} = 0$, gives
...
Example 15.6: The eccentricity vector
...
$$
\{A_i, A_j \} = −2 \left(\frac{\mathbf{p}^2}{2\mu} + \frac{k}{ r} \right) \epsilon_{ijk}L_k \tag{a}\label{15-a-1}
$$

Note that the bracket on the right-hand side of Equation \ref{15-a-1} equals the Hamiltonian $H$ for the inverse square-law attractive force, and thus the Poisson bracket equals
...
That is, the eccentricity vector commutes with the Hamiltonian and thus it is a constant of motion. Previously this result was obtained directly using the equations of motion as given in equation $11.8.36$. Note that the three constants of motion, $L$, $A$ and $H$ form a closed algebra under the Poisson Bracket formalism similar to the triad of constants of motion, $L$, $A^{\prime}$ and $H$ that occur for the two-dimensional, isotropic linear oscillator described above. Examples 15.5 and 15.6 illustrate that the Poisson Brackets representation of Hamiltonian mechanics is a powerful probe of the underlying physics, as well as confirming the results obtained directly from the equations of motion as described in chapter $11.8$ and $11.9$.
...
$1$: Infinitessimal element of area in phase space
:::
...
The generating function $F$ can be any well-behaved function with continuous second derivatives of both the old and new canonical variables $\mathbf{p}$, $\mathbf{q}$, $\mathbf{P}$, $\mathbf{Q}$ and $t$. Thus the integrands of \ref{15.73} and \ref{15.74} are related by
...
Insert Equation \ref{15.77} into Equation \ref{15.76}, and assume that the trivial scale factor $\lambda = 1$, then
...
Insert this into Equation \ref{15.76}, and assume that the trivial scale factor $\lambda = 1$, then
...
Note the special case when $\mathcal{H}(\mathbf{Q},\mathbf{P}, t)=0$, then Equation \ref{15.89} has been reduced to the Hamilton-Jacobi relation \ref{15.11}

$$
H(\mathbf{q},\mathbf{p}, t) + \frac{\partial S}{\partial t} = 0 \label{15.11}
$$

In this case, the generating function $F$ determines the action functional $S$ required to solve the Hamilton-Jacobi equation $(15.4.23)$). Since Equation \ref{15.89} has transformed the Hamiltonian $H(\mathbf{q},\mathbf{p}, t) \rightarrow \mathcal{H}(\mathbf{Q},\mathbf{P}, t)$, for which $\mathcal{H}(\mathbf{Q},\mathbf{P}, t)=0$, then the solution $\mathbf{Q}(t), \mathbf{P}(t)$ for the Hamiltonian $\mathcal{H}(\mathbf{Q},\mathbf{P}, t)=0$ is obtained easily. This approach underlies Hamilton-Jacobi theory presented in chapter $15.4$.
...
Example 15.1: The identity canonical transformation

The identity transformation $F_2(\mathbf{q},\mathbf{P}) = \mathbf{q} \cdot \mathbf{P}$ satisfies \ref{15.89} if the following relations are satisfied $p_i = \frac{\partial F_2 }{\partial q_i }= P_i$, $Q_i = \frac{\partial F_2}{ \partial P_i } = q_i$, $\mathcal{H}=H$. Note that the new and old coordinates are identical, hence $F_2 = q_iP_i$ generates the identity transformation $q_i = Q_i , p_i = P_i$.

Example 15.2: The point canonical transformation

Consider the point transformation $F_2(\mathbf{q} \cdot \mathbf{P}) = f(\mathbf{q},t)\cdot \mathbf{P}$ where $f(\mathbf{q},t)$ is some function of $\mathbf{q}$. This transformation satisfies \ref{15.89} if the following relations are satisfied $Q_i = \frac{\partial F_2}{ \partial P_i} = f_i(q_i)$, $p_i = \frac{\partial F_2}{\partial q_i } = \frac{\partial f_i(q_i,t)}{ \partial q_i }$, $\mathcal{H}=H$. Point transformations correspond to point-to-point transformations of coordinates.

Example 15.3: The exchange canonical transformation

The identity transformation $F_1(\mathbf{q}, \mathbf{Q}) = \mathbf{q} \cdot \mathbf{Q}$ satisfies \ref{15.89} if the following relations are satisfied $p_i = \frac{\partial F_1}{ \partial q_i} = Q_i$, $P_i = −\frac{\partial F_1} {\partial Q_i} = −q_i$, $\mathcal{H}=H$ That is, the coordinates and momenta have been interchanged.

Example 15.4: Infinitessimal point canonical transformation
...
satisfies \ref{15.89} if the following relations are satisfied
...
Example 15.5: 1-D harmonic oscillator via a cononical transformation
...
$$
q = \sqrt{\frac{2P}{ m\omega}} \sin Q \tag{a}\label{15-a-2}
$$

$$
p = \sqrt{2m\omega P} \cos Q \tag{b}\label{15-b-1}
$$
...
Substituting $Q$ into \ref{15-a-2} gives the well known solution of the one-dimensional harmonic oscillator
...
$$
H(\mathbf{q},\mathbf{p}, t) + \frac{\partial S}{\partial t} = 0 \label{15.11-2}
$$
...
Table 15.1 summarizes the time-dependent and time-independent forms of the Hamilton-Jacobi equation.
...
$$
\frac{1}{ 2m } \nabla^2S + \frac{\partial S}{\partial t} = 0 \tag{A}\label{15-A}
$$
...
$$
S = \boldsymbol{\alpha} \cdot \mathbf{ r} − Et \tag{B}\label{15-B}
$$

For Equation \ref{15-B} to be a solution of Equation \ref{15-A} requires that

$$
E = \frac{1}{ 2m} \boldsymbol{\alpha}^2 \tag{C}\label{15-C}
$$

Therefore

$$
S = \boldsymbol{\alpha} \cdot \mathbf{r} − \frac{1}{ 2m} \boldsymbol{\alpha}^2t \tag{D}\label{15-D}
$$
...
Example 15.2: Point particle in a uniform gravitational field
...
Example 15.3: One-dimensional harmonic oscillator
...
Example 15.4: The central force problem
...
Example 15.5: Linearly-damped, one-dimensional, harmonic oscillator
...
$$
\frac{m}{ 2} [ \ddot{q}+ \Gamma \dot{q} + \omega^2_0 q ] = 0 \tag{a}\label{15-a-3}
$$
...
$$
L_2 (q, \dot{q}, t ) = \frac{m}{ 2} e^{\Gamma t} [ \dot{q}^2 − \omega^2_0 q^2] \tag{b}\label{15-b-2}
$$

This Lagrangian gave the generalized momentum to be

$$
p = \frac{\partial L^2}{ \partial \dot{q}} = m\dot{q} e^{\Gamma t} \tag{c}\label{15-c-1}
$$
...
$$
S(q, P, t) = F_2(q, P, t) = qPe^{ \frac{\Gamma t}{ 2}} = QP \tag{d}\label{15-d2}
$$

Then the canonical transformation gives

$$
p = \frac{\partial S}{ \partial q} = P e^{\frac{ \Gamma t}{ 2}} \label{15-e}\tag{e}
$$
...
$$
\mathcal{H}(Q, P, t) = H_2(q, p, t) + \frac{\partial F_2}{ \partial t} = \frac{P^2}{ 2m} + \frac{\Gamma}{ 2} QP + \frac{m\omega^2_0}{ 2} Q^2 \tag{f}\label{15-f}
$$
...
$$
\frac{\partial S}{ \partial Q} = P \label{15-g}\tag{g}
$$

Use Equation \ref{15-g} to substitute for $P$ in the Hamiltonian $\mathcal{H}(Q, P, t)$ (Equation \ref{15-f}), then the Hamilton-Jacobi method gives
...
$$
\left[ \frac{1}{2m} \left(\frac{\partial W }{\partial Q} \right)^2 + \Gamma Q\frac{\partial W}{ \partial Q} + \frac{m\omega^2_0}{ 2} Q^2 \right] = \alpha \tag{h}\label{15-h}
$$
...
then Equation \ref{15-h} can be written as
...
$$
\omega = \omega_0 C = \omega_0 \sqrt{1 − \left( \frac{\Gamma }{2\omega_0} \right)^2} = \sqrt{ \omega^2_0 − \left(\frac{\Gamma }{2} \right)^2} \label{15-m}\tag{m}
$$

Transforming back to the original variable $q$ gives

$$
q(t) = Ge^{−\frac{ \Gamma t }{2}} \sin (\omega t + \delta ) \tag{n}\label{15-n}
$$

where $G$ and $\delta$ are given by the initial conditions. Equation \ref{15-m} is identical to the solution for the underdamped linearly-damped linear oscillator given previously in equation $(3.5.12)$.
...
$1$: Surfaces of constant action integral S (dashed lines) and the corresponding particle momenta (solid lines) with arrows showing the direction.
:::
...
Then the Hamiltonian, \ref{15.118} equals
...
The solution of equations \ref{15.122} and \ref{15.123} is of the form
...
where $C$, and $t_0$ are integration constants. For the harmonic oscillator, equations \ref{15.124} and \ref{15.125} correspond to the usual elliptical contours in phase space, as illustrated in Figure 15.1.

:::{figure} ../images/lt-21256-14.5.1.png
:alt: 14.5.1.PNG

$1$: The potential energy $V (q)$, (upper) and corresponding phase space $(p,q)$ (middle) for the harmonic oscillator at four equally spaced total energies $E$. The corresponding action-angles $(I \phi)$ resulting from a canonical transformation of this system are shown in the lower plot.
:::
...
where $I$ is defined by Equation \ref{15.117} and the angle $\phi$ being the corresponding canonical angle. The logical approach to this canonical transformation for the harmonic oscillator is to define $q$ and $p$ in terms of $\phi$ and $I$
...
The momentum $p_i (q_i, \alpha_1, \alpha_2, ..\alpha_n)$ represents the trajectory of the system in the $(q_i, p_i)$ phase space that is characterized by Hamilton’s characteristic function $W(q,J)$. Combining equations \ref{15.116}, \ref{15.136} gives
...
The derivative with respect to $q_i$ vanishes except for the $W_j$ component of $W$. Thus Equation \ref{15.143} reduces to
...
Example 15.1: Adiabatic invariance for the simple pendulum
...
$$
\Delta W = T\Delta L = −Mg\Delta L − Mg \frac{\theta^2_0}{4} \Delta L \label{15-a-4}\tag{a}
$$

while the change in internal oscillator energy is

$$
\Delta(−Mg L \cos \theta_0) = \Delta \left[ −Mg L(1 − \frac{\theta^2_0}{2} ) \right] = −Mg\Delta L + \frac{1}{ 2} Mg\Delta( L\theta^2_0) = −Mg\Delta L + \frac{1}{ 2} Mg\theta^2_0\Delta L + Mg L\theta_0 \Delta\theta_0 \label{15-b-3}\tag{b}
$$
...
$$
(\theta_0 L^{\frac{3}{ 4}} ) = \text{ constant} \label{15-c-2}\tag{c}
$$
...
where that last step is due to Equation \ref{15-c-2}.
...
$$
\mathcal{H}(Q_i, P_i, t) = H_0(q_1, ...q_n; \frac{\partial S}{\partial q_1} ..., \frac{\partial S}{\partial q_n };t) + \frac{\partial S}{\partial t} = 0 \label{15.90-2}
$$
...
Example 15.2: Lindblad resonance in planetary and galactic motion
...
The indefinite integral of this equation reproduces the definite integral \ref{15.1} to within an arbitrary constant, i.e.
